/**
   ...
   * Follow https://instagram.com/fbryaaannn
*/

require('./config')
const { default: dantaConnect, useSingleFileAuthState, DisconnectReason, fetchLatestBaileysVersion, generateForwardMessageContent, prepareWAMessageMedia, generateWAMessageFromContent, generateMessageID, downloadContentFromMessage, makeInMemoryStore, jidDecode, proto } = require("@adiwajshing/baileys")
...
async function startdanta() {
    const danta = dantaConnect({
        ...
        browser: ['𝑫𝒂𝒏𝒕𝒂 𝑫𝒆𝒗','Fire Fox','1.0.0'],
        auth: state
    })
    ...
    let pa7rick = await danta.sendContact(callerId, global.owner)
    danta.sendMessage(callerId, { text: `*Sistem otomatis block!*\n*Jangan menelpon bot*!\n*Silahkan Hubungi Owner Untuk Dibuka !*`}, { quoted : pa7rick })
    await sleep(8000)
    ...
    // Group Update
    danta.ev.on('groups.update', async pea => {
       //console.log(pea)
    // Get Profile Picture Group
       try {
       ppgc = await danta.profilePictureUrl(pea[0].id, 'image')
       } catch {
       ppgc = 'https://shortlink.hisokaarridho.my.id/rg1oT'
       }
       let wm_fatih = { url : ppgc }
       if (pea[0].announce == true) {
       danta.send5ButImg(pea[0].id, `「 *Group Settings Change* 」\n\nGroup telah ditutup oleh admin, Sekarang hanya admin yang dapat mengirim pesan !`, `Group Settings Change Message by 𝑫𝒂𝒏𝒕𝒂 𝑫𝒆𝒗`, wm_fatih, [])
       } else if(pea[0].announce == false) {
       danta.send5ButImg(pea[0].id, `「 *Group Settings Change* 」\n\nGroup telah dibuka oleh admin, Sekarang peserta dapat mengirim pesan !`, `Group Settings Change Message by 𝑫𝒂𝒏𝒕𝒂 𝑫𝒆𝒗`, wm_fatih, [])
       } else if (pea[0].restrict == true) {
       danta.send5ButImg(pea[0].id, `「 *Group Settings Change* 」\n\nInfo group telah dibatasi, Sekarang hanya admin yang dapat mengedit info group !`, `Group Settings Change Message by 𝑫𝒂𝒏𝒕𝒂 𝑫𝒆𝒗`, wm_fatih, [])
       } else if (pea[0].restrict == false) {
       danta.send5ButImg(pea[0].id, `「 *Group Settings Change* 」\n\nInfo group telah dibuka, Sekarang peserta dapat mengedit info group !`, `Group Settings Change Message by 𝑫𝒂𝒏𝒕𝒂 𝑫𝒆𝒗`, wm_fatih, [])
       } else {
       danta.send5ButImg(pea[0].id, `「 *Group Settings Change* 」\n\nGroup Subject telah diganti menjadi *${pea[0].subject}*`, `Group Settings Change Message by 𝑫𝒂𝒏𝒕𝒂 𝑫𝒆𝒗`, wm_fatih, [])
     }
    })
            ...
            for (let num of participants) {
                ...
                let dmember = metadata.participants.length
                   let ppusr = await getBuffer(ppuser)
                	...
                	let buff = await getBuffer (`https://didinbotz-apiii.herokuapp.com/api/canvas/welcome?name=${num.split("@")[0]}&mem=${dmember}&gcname=${metadata.subject}&picurl=${bup}&bgurl=https://w0.peakpx.com/wallpaper/11/65/HD-wallpaper-anime-girl-loli-cute-brown-hair-twintails-anime.jpg&apikey=925n2c494`)
                    var button = [
                          { 
                            buttonId: `tesrow`, 
                            buttonText: { 
                ...
                await danta.sendMessage(anu.id, { 
                   caption: `
✑ Halo👋 @${num.split("@")[0]},

*✑ Welcome To:* ${metadata.subject}

✑ Jangan Lupa Isi Di bawah ini
...
✑ Askot :
  ...
  Note : Jangan Lupa Kalian Kenalan sama member baru ya, jangan di kacangin kasihan :v
...
✑ *_Semoga Betah Ya Di Group Ini_*`, 
                 ...
                 let dmember = metadata.participants.length
                	...
                	let buff2 = await getBuffer (`https://didinbotz-apiii.herokuapp.com/api/canvas/goodbye?name=${num.split("@")[0]}&mem=${dmember}&gcname=${metadata.subject}&picurl=${bup2}&bgurl=https://w0.peakpx.com/wallpaper/11/65/HD-wallpaper-anime-girl-loli-cute-brown-hair-twintails-anime.jpg&apikey=9267ic6a0f1`)
                	var button = [
                          { 
                            buttonId: `tesrow`, 
                            buttonText: { 
                              displayText: `Selamat Tinggal Kawan Terbaikku 😥👋` 
                            }, type: 1 
                          }
                       ]
                await danta.sendMessage(anu.id, { 
    ...
    danta.ev.on('connection.update', async (update) => {
        ...
        let reason = new Boom(lastDisconnect?.error)?.output.statusCode
            ...
            else if (reason === DisconnectReason.connectionClosed) { console.log("Connection closed, reconnecting...."); startdanta(); }
            else if (reason === DisconnectReason.connectionLost) { console.log("Connection Lost from Server, reconnecting..."); startdanta(); }
            ...
            else if (reason === DisconnectReason.restartRequired) { console.log("Restart Required, Restarting..."); startdanta(); }
            else if (reason === DisconnectReason.timedOut) { console.log("Connection TimedOut, Reconnecting..."); startdanta(); }
    ...
    // Add Other
      ...
      * @param {*} options
      */
     danta.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
      let mime = '';
      let res = await axios.head(url)
      mime = res.headers['content-type']
      if (mime.split("/")[1] === "gif") {
     return danta.sendMessage(jid, { video: await getBuffer(url), caption: caption, gifPlayback: true, ...options}, { quoted: quoted, ...options})
      }
      let type = mime.split("/")[0]+"Message"
      if(mime === "application/pdf"){
     return danta.sendMessage(jid, { document: await getBuffer(url), mimetype: 'application/pdf', caption: caption, ...options}, { quoted: quoted, ...options })
      }
      if(mime.split("/")[0] === "image"){
     return danta.sendMessage(jid, { image: await getBuffer(url), caption: caption, ...options}, { quoted: quoted, ...options})
      }
      if(mime.split("/")[0] === "video"){
     return danta.sendMessage(jid, { video: await getBuffer(url), caption: caption, mimetype: 'video/mp4', ...options}, { quoted: quoted, ...options })
      }
      if(mime.split("/")[0] === "audio"){
     return danta.sendMessage(jid, { audio: await getBuffer(url), caption: caption, mimetype: 'audio/mpeg', ...options}, { quoted: quoted, ...options })
      }
      }
